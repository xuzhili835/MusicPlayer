...
### 🔒 隐私模式与老板键
- **一键隐私**：全局快捷键（默认 `F9`，可自定义）随时触发——即使你在游戏或其他全屏应用中
- 触发后立即暂停播放，全屏遮罩盖住整个窗口，**歌曲、歌单、封面等任何信息都不显示**
- 再按一次恢复（保持暂停，不会突然出声），触发动作可选「仅遮罩」或「遮罩+最小化」
- 桌面歌词窗口联动隐藏
...
### 🎵 内容管理与分类
- 内容分三类：**音乐 / 播客 / 听力**，右键随时改分类，侧边栏分区浏览
- 歌单管理：创建/重命名/删除，条目跨歌单管理
- 最近播放历史、搜索、本地文件导入

### 📥 音源下载（Bilibili + YouTube）
- 统一入口粘贴链接自动识别（支持 B 站分享文本 / BV号 / YouTube 各种链接格式）
- 内置 yt-dlp + ffmpeg，自动下载最佳音质并转 MP3
- 自动下载字幕转歌词（zh-Hans / zh / en）
- **网络设置**：代理地址（下载 YouTube 必需）、cookies.txt（年龄限制内容）

### 🖊️ 歌词：下载 / AI 转写 / 图片 OCR
- **AI 识别歌词（音频转文字）**：本地 whisper 模型，中/英/日等 99 种语言自动检测，输出带时间轴 LRC
  - 模型四档规格（75MB ~ 1.5GB），**由你选择下载**，随时删除释放空间
