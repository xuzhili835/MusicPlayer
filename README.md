...
4. **AI 歌词**：设置 → AI 歌词识别可直接管理模型（推荐 Small）或批量识别；右键任意条目也可单首识别，缺模型时会引导下载
...
### 工具太旧 / 下载报错？
检查控制台 → 「检查 yt-dlp 更新」一键升级（站点风控演进快，建议偶尔更新）。
...
不会。模型必须你在设置 → AI 歌词识别中确认规格并点击下载才会下载；非当前模型可随时删除。
