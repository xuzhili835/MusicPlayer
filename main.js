...
class BiliMusicPlayer {
    ...
    // B站对无 cookie 的 API 请求会返回 412（2024 年起逐步收紧的风控，与 IP/时段相关，
    // 这就是"以前能下现在不行"的原因）。解法：先调 B站公开的 finger/spi 接口领一组
    // 匿名 buvid3/buvid4 cookie（社区验证的方案），随请求一起发送
    async getNetworkArgs(url = '') {
        ...
        try {
            ...
            if (cookiesPath && typeof cookiesPath === 'string' && cookiesPath.trim()) {
                ...
                // B站：注入匿名 buvid cookie 规避 412（失败则降级为无 cookie）
                try {
                    const biliCookieFile = await this.getBilibiliCookies();
                    if (biliCookieFile) args.push('--cookies', biliCookieFile);
                } catch (e) {
                    console.warn('获取B站匿名 cookie 失败（继续尝试无 cookie 请求）:', e.message);
                }
            }
        } catch (error) {
    ...
    // 获取B站匿名 cookie（buvid3/buvid4），缓存 1 小时
    async getBilibiliCookies() {
        const now = Date.now();
        if (this._biliCookieFile && this._biliCookieTime && (now - this._biliCookieTime) < 3600 * 1000) {
            return this._biliCookieFile;
        }

        const https = require('https');
        const body = await new Promise((resolve, reject) => {
            const req = https.get('https://api.bilibili.com/x/frontend/finger/spi', {
                headers: {
        ...
        const spi = JSON.parse(body);
        if (spi.code !== 0 || !spi.data || !spi.data.b_3 || !spi.data.b_4) {
            throw new Error('spi 接口返回异常');
        }

        // 写成 Netscape 格式 cookies 文件给 yt-dlp
        const cookiePath = path.join(this.tempDir, 'bilibili_cookies.txt');
        const expire = Math.floor(now / 1000) + 365 * 24 * 3600;
        const content = [
            '# Netscape HTTP Cookie File',
            `# Generated by Sakura Echo at ${new Date().toISOString()}`,
            `.bilibili.com\tTRUE\t/\tFALSE\t${expire}\tbuvid3\t${spi.data.b_3}`,
            `.bilibili.com\tTRUE\t/\tFALSE\t${expire}\tbuvid4\t${spi.data.b_4}`
        ].join('\n') + '\n';
        await fs.writeFile(cookiePath, content, 'utf8');

        this._biliCookieFile = cookiePath;
        this._biliCookieTime = now;
        console.log('B站匿名 cookie 已刷新:', cookiePath);
        return cookiePath;
    }
    ...
    // 智能解析文件名元数据：剥装饰性前缀/曲号，按 "A - B" 拆出 艺术家/标题
    // 例："【4K Hi-Res】ヨルシカ - 春泥棒 [MV].mp3" → 艺术家: ヨルシカ, 标题: 春泥棒
    parseFileNameMeta(fileName) {
        let stem = path.basename(fileName, path.extname(fileName)).trim();
        ...
        // 剥掉曲号前缀："01. " "01 - " "01_" "Track 01 - "
        stem = stem.replace(/^track\s*\d+[\s._-]*/i, '');
        stem = stem.replace(/^\d{1,3}[\s._-]+\s*/, '').trim();

        // 剥掉结尾的发行标注：(Official MV) [Live] 【MV】［PV］（full） 等（含全角括号）
        stem = stem.replace(/[\s._-]*[（(【\[［「『]?(official|mv|music\s*video|live|pv|ncop|nced|full)[^）)】\]」』]*[）)】\]」］』]*\s*$/i, '').trim();

        // 按分隔符拆艺术家与标题：支持 "A - B"、"A – B"、"A_-_B"（B站转存的下划线风格）
        ...
        return { artist: '', title: stem || path.basename(fileName, path.extname(fileName)) };
    }

    // 音频元数据提取
    async extractAudioMetadata(filePath) {
        // 文件名智能解析结果（作为标签缺失时的兜底）
        const parsed = this.parseFileNameMeta(filePath);
        ...
        try {
            ...
            return {
                ...
                artist: common.artist || common.albumartist || parsed.artist,
                album: common.album || '',
