...
window.LyricsUI = {
    ...
    async showOcrImportDialog(song) {
        const imagePath = await electronAPI.ocr.selectImage();
        if (!imagePath) return;
            ...
            <div class="modal-content" style="width: 520px;">
                ...
                <div class="modal-body">
                    <p style="font-size: 12px; color: var(--text-faint); margin-bottom: 10px;">
                        识别结果可编辑；支持 LRC 时间标签（[mm:ss.xx]），无标签将按纯文本保存
                    </p>
                    <textarea id="ocr-result" class="lyrics-textarea" disabled placeholder="正在识别..."></textarea>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" data-role="cancel">取消</button>
                    <button class="btn btn-primary" id="save-ocr-lyrics-btn" disabled>保存</button>
                </div>
            </div>
        `;
        document.body.appendChild(dialog);

        const textarea = dialog.querySelector('#ocr-result');
        const saveBtn = dialog.querySelector('#save-ocr-lyrics-btn');
        ...
        const result = await electronAPI.ocr.recognizeImage(imagePath);
